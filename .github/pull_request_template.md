...
<!-- How does this land on the box (/srv/markescence, reverse-proxied by central Caddy)?
     Tick what applies, delete the rest. -->
- [ ] No box-side action needed — a normal `make deploy` rebuild is enough
- [ ] Needs a redeploy: `cd /srv/markescence && make deploy` (or `make remote-deploy`)
...
<!-- How did you test this? e.g. `make verify` output, `curl -sI https://markescence.msge.no`,
...
- [ ] Compose project name still pinned (`name: markescence`) so a dir rename can't orphan volumes
